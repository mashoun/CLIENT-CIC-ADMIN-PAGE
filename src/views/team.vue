        ...
        <div class="col-12">
          <div class="table-responsive">
            <table class="table caption-top">
              ...
              <tbody>
                <tr v-for="node in store.profile.team" :key="node">
  ...
  export default {
    ...
    methods:{
        ...
        if(confirm('Are you sure?')){
            ...
            if(files){
              ...
              for(let i = 0 ; i < files.length ; i++){
                  files64.push({
                      alt:`Team-Thumbnail-${utilities.getCurrentDate()}`,
              ...
              var position = prompt('Enter Team Member Position')
              var description = prompt('Enter Team Member Description')
  
              fetch(this.store.getApi() + "?addTeamRecord=1",{
                method:"POST",
                headers:{
                  "Content-Type":"text/plain"
                },
      ...
      removePartnerRecord(id){
        if(confirm('Are you sure ?')){
          this.spinner = true
          fetch(this.store.getApi() + "?removeTeamRecord=1",{
            method:"POST",
            headers:{
              "Content-Type":"text/plain"
            },
